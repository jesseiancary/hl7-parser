...
export default class HL7 {
  ...
  public async create(req, h): Promise<any> {
    ...
    const hl7Document = {
      ...req.payload,
      owner: req.auth.artifacts.user._id
    };
  ...
  public async find(req, h): Promise<any> {
    ...
    return HL7Model.find({ owner: req.auth.artifacts.user._id }).populate({ path: 'owner', select: '-password -salt' })
    .then(hl7 => {
  ...
  public async findOne(req, h): Promise<any> {
    ...
    return HL7Model.findOne({ _id: req.params.id, owner: req.auth.artifacts.user._id }).populate({ path: 'owner', select: '-password -salt' })
    .then(hl7 => {
  ...
  public async update(req, h): Promise<any> {
    ...
    .then(jsonData => {
      ...
      return HL7Model.findOneAndUpdate({ _id: req.params.id, owner: req.auth.artifacts.user._id }, { hl7_data: req.payload.hl7_data, json_data: jsonData }, { new: true }).populate({ path: 'owner', select: '-password -salt' })
      .then(hl7_and_json => {
